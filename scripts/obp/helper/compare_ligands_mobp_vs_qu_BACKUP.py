...
def read_mobp_data(mobp_path):
	...
	mobp4_sorted_pki 	= mobp4[mobp4[:,9].argsort()] 	# sorted by x-th column
	mobp4_sorted_le 	= mobp4[mobp4[:,10].argsort()] 	# sorted by x-th column
	mobp4_sorted_fq 	= mobp4[mobp4[:,11].argsort()] 	# sorted by x-th column
	mobp4_sorted_sile 	= mobp4[mobp4[:,12].argsort()] 	# sorted by x-th column
...
def compare_ligand_sequence(mobp_input,qobp_input,intersection_mobp):
...
qu2=[i.lower() for i in qu]
mobp2=[i.lower() for i in mobp]
...
# then accounting for chemicals that are just slightly differentl spelt
...
# correct the spelling of ligands to mOBP mode
intersection_mobp = []
...
# v = venn2(subsets = (len(qu2), len(mobp2), len(intersection)))

# v.get_label_by_id('A').set_text('Exp')
# v.get_label_by_id('B').set_text('MD')


# plt.savefig("../../data/obp_input/venn_mobp_vs_qaio.png")
...
for m_path, q_path in mq_paths:
	...
	mobpX_ligands, qobpX_ligands, qu_raw = compare_ligand_sequence(m_path,q_path,intersection_mobp)
